...
/** Discovery file written to ~/.cc-monitor/server.json so hook scripts can find the server. */
export interface ServerConfig {
  ...
  startedAt: number;
  /** Host the server is bound to (e.g. '127.0.0.1' or '0.0.0.0'). Absent in
   *  legacy server.json files — hook scripts fall back to 127.0.0.1. */
  host?: string;
  /**
 ...
 *
 * Discovery: writes `~/.cc-monitor/server.json` with port, PID, and auth token.
...
export class PixelAgentsServer {
  ...
  async start(options?: {
    ...
    embedded?: boolean;
    host?: string;
    /** Address hook scripts should POST to (written to server.json). Defaults
     *  to `host`. Set this when binding 0.0.0.0 but hooks need a real IP. */
    hookHost?: string;
    port?: number;
    staticDir?: string;
    ...
    if (existing && isProcessRunning(existing.pid)) {
      ...
      console.log(
        `[cc-monitor] Reusing existing server on port ${existing.port} (PID ${existing.pid})`,
      );
      return existing;
    }
    ...
    this.config = {
      ...
      token,
      startedAt: Date.now(),
      host: options?.hookHost ?? options?.host ?? '127.0.0.1',
    ...
    console.log(`[cc-monitor] Server: listening on ${this.config.host}:${port}`);
  ...
  /** Returns the absolute path to ~/.cc-monitor/server.json. */
  ...
  private writeServerJson(config: ServerConfig): void {
    ...
    try {
      ...
      console.error(`[cc-monitor] Failed to write server.json: ${e}`);
    }
  }
